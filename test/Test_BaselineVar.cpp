...
int main(int argc, char *argv[])
{
    ...
    std::string folderName = (lastSlash != std::string::npos) ? folderPath.substr(lastSlash + 1) : folderPath;
    // Construct the data file name
    std::string dataFileName = folderPath + "/FILTERED/DataF_" + folderName + ".root";
    ...
    TH1F *hBaseLine = new TH1F("hBaseLine", "Baseline Distribution", 100000, 0, 100);

    for (int i = 0; i < nentries; i++)
    {
        double baseline = hitsVector[i]->GetWFPtr()->GetBaseLine();
        float timeStamp = hitsVector[i]->GetTimestamp() / 1E12; // in  seconds
        // std::cout << "timeStamp: " << timeStamp << " baseline: " << baseline << std::endl;
