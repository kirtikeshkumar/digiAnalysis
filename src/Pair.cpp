...
Pair::Pair(const singleHits *h1, const singleHits *h2) {
  hit1 = std::make_unique<digiAnalysis::singleHits>(*h1);
  hit2 = std::make_unique<digiAnalysis::singleHits>(*h2);
  if (hit1 && hit2) {
    ...
    PairDelTime = hit1->GetChNum() > hit2->GetChNum()
                      ? hit2->GetTimestamp() - hit1->GetTimestamp()
                      : hit1->GetTimestamp() - hit2->GetTimestamp();
  } else {
...
Pair::Pair(const Pair &other)
    : hit1(other.hit1 ? std::make_unique<digiAnalysis::singleHits>(*other.hit1)
                      : nullptr),
      hit2(other.hit2 ? std::make_unique<digiAnalysis::singleHits>(*other.hit2)
                      : nullptr),
      SumEnergy(other.SumEnergy), PairDelTime(other.PairDelTime) {}
...
singleHits *Pair::GetHitPtr(Short_t Sel) {
  if (!hit1 || !hit2) {
  ...
  switch (Sel) {
  case -1:
    return (hit1->GetTimestamp() <= hit2->GetTimestamp()) ? hit1.get()
                                                          : hit2.get();
  case -2:
    return (hit1->GetTimestamp() > hit2->GetTimestamp()) ? hit1.get()
                                                         : hit2.get();
  case 0:
    return (hit1->GetChNum() <= hit2->GetChNum()) ? hit1.get() : hit2.get();
  case 1:
    return (hit1->GetChNum() > hit2->GetChNum()) ? hit1.get() : hit2.get();
  default:
    return nullptr;
...
Long64_t Pair::GetPairDelTime() { return PairDelTime; }
...
double Pair::GetPairEnergy(double fac1, double fac2) {
  return GetPairHitEnergy(0) * fac1 + GetPairHitEnergy(1) * fac2;
}
