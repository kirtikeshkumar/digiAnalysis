...
namespace digiAnalysis
{
  ...
  void Analysis::CreatePairs()
  {
    ...
    */
    ...
    if (singleChannel)
    {
      ...
      {
        if (vecOfHits[iter + 1]->GetTimestamp() - vecOfHits[iter]->GetTimestamp() < PairCoincWindow * 1000)
        {
          coincPair.ClearPair();
          coincPair.SetPair(vecOfHits[iter].get(),
                            vecOfHits[iter + 1].get());
    ...
    else
    {
      ...
      while (!isEnd)
      {
        ...
        if ((minTime < PairCoincWindow * 1000) and (startChannel != stopChannel))
        {
          coincPair.ClearPair();
  ...
  std::vector<Pair *> Analysis::GetPairsVec() { return vecOfPairs; }
