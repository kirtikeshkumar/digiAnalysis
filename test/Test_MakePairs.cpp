...
int main(int argc, char *argv[]) {
  ...
  std::vector<std::unique_ptr<digiAnalysis::singleHits>> &hitsVector =
      an.GetSingleHitsVec();
  std::vector<digiAnalysis::singleHits> hitsVectorCh0 = an.GetSingleHitsVec(2);
  std::vector<digiAnalysis::singleHits> hitsVectorCh1 = an.GetSingleHitsVec(3);
  ...
  TH1F *hICDelT =
      new TH1F("hICDelT", "inter channel nearest event time difference", 20000,
               -10000, 10000);
  ...
  /* while (i < hitsVectorCh0.size()) {
     ...
     // std::cout << "Ch: " << hitsVectorCh0[i].GetChNum() << " T0: " << Ch0Time
     //           << std::endl;
     TDiffMin = 1000000000000;
     while (j < hitsVectorCh1.size()) {
       ...
       // Ch1Time = hitsVectorCh1[j].GetTimestamp();
       // std::cout << "Ch: " << hitsVectorCh1[j].GetChNum() << " T1: " <<
       // Ch1Time
       //           << std::endl;
