...
int main(int argc, char *argv[])
{
  ...
  // std::string fname = "/media/kirtikesh/Ventoy/GGAG/DataF_CH1@V1730_167_GGAG_2inch_insideHutch_3Oct_4hr.root";
  ...
  UShort_t wfSz = 0;
  Long64_t start = 21000;
  UShort_t count = 50;
  for (Long64_t i = start; i < start + count; i++)
  {
    if (hitsVector[i]->GetPSD() < 1.35)
    {
  ...
  // test plot data
  TH2 *hEnPSD = new TH2F("hEnPSD", "Energy(x) vs PSD(y)", 8192, 0, 8192, 819, 0, 1);
  std::cout << "hEnPSD created" << std::endl;
  // TH2 *hNumPSD = new TH2F("hNumPSD", "Index(x) vs PSD(y)", nentries / 100, 0, nentries, 819, 0, 1);
  TH2 *hNumEn = new TH2F("hNumEn", "Index(x) vs Energy(y)", nentries / 100, 0, nentries, 8192, 0, 8192);
