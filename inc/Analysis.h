...
class Analysis {
  /**
   * @class Analysis
   * @brief Class for analyzing event data from files, applying energy
   * thresholds, and managing hit and pair data.
   */
private:
  std::vector<std::unique_ptr<singleHits>> vecOfHits;
  std::vector<std::unique_ptr<Pair>> vecOfPairs;
...
public:
  ...
  /**
   * @brief Constructs an Analysis object for processing data from a specified
   * file.
   *
   * @param datafilename The path to the data file to be analyzed.
   * @param numOfEvents The number of events to process from the data file. If
   * set to 0, all events are processed. (Default: 0)
   * @param EThreshold The energy threshold to apply during analysis. Events
   ...
   * @param numOfEvents    The number of events to process (default is 0, which
   * means all events).
   * @param EThreshold     The energy threshold for event selection (default is
   * 0).
   */
  ...
  /**
   * @brief Constructs an Analysis object to process event data from a file for
   * a specific channel.
   *
   * @param datafilename   The path to the data file to be analyzed.
   * @param channel        The Channel to be read.
   * @param start          The index of the first event to process. (Default: 0)
   * @param numOfEvents    The number of events to process (default is 0,
   * which means all events).
   * @param EThreshold     The energy threshold for event selection (default
   * is 0).
   */
  Analysis(UShort_t channel, std::string datafilename, ULong64_t start,
           ULong64_t numOfEvents = 0, double EThreshold = 0);
   ...
   * @param EThreshold The minimum energy threshold to apply when loading
   * events.
   */
  void LoadData(ULong64_t numOfEvents, double EThreshold);

  /**
   * @brief Loads data for analysis from a specified starting event, applying an
   * energy threshold.
   *
  ...
  /**
   * @brief Sorts the vector of hits based on the specified major and optional
   * minor criteria.
   *
  ...
  /**
   * @brief Returns a vector of pointers to singleHits objects for a specific
   * channel.
   *
  ...
  /**
   * @brief Returns a unique pointer to a singleHits object at the specified
   * index.
   *
  ...
  /**
   * @brief Returns a reference to the vector of unique pointers to Pair
   * objects.
   *
   * @return Reference to the vector of unique pointers to Pair.
   */
  std::vector<std::unique_ptr<Pair>> &GetPairsVec();
};
