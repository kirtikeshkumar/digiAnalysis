...
protected:
  ...
  TPolyLine *DrawGate(UShort_t start, UShort_t stop, double Ht);
...
public:
  ...
  void SetBaseLine(const std::vector<double> tr, int start, int nSample);
  void SetTracesFFT();
  ...
  void Plot();
  void Plot(std::vector<double> tr, const std::string &option = "CLEAR");
  ...
  double EvalNoisePar2(int startTime = GateStart,
                       int stopTime = GateStart + GateLenLong);
