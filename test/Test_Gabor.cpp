...
int main(int argc, char *argv[]) {
  ...
  for (evi = 0; evi < nentries && keepGoing; ++evi) //
  {
    ...
    if (hitsVector[evi]->GetChNum() == 9 and
      ...
      TH2 *hTracesShort =
          new TH2F("hTracesShort", "Short Time Traces", rangeSTFT, 0, rangeSTFT,
                   kernelSz, 0, kernelSz);
      TH2 *hSTFT =
          new TH2F("hSTFT", "Short Time Fourier Transform", rangeSTFT, 0,
                   rangeSTFT, kernelSz / 2 + 1, 0, kernelSz / 2 + 1);
      TH2 *hSTFT_Re = new TH2F(
          "hSTFT_Re", "Short Time Fourier Transform Real Part", rangeSTFT, 0,
          rangeSTFT, kernelSz / 2 + 1, 0, kernelSz / 2 + 1);
      TH2 *hSTFT_Im = new TH2F(
          "hSTFT_Im", "Short Time Fourier Transform Imaginary Part", rangeSTFT,
          0, rangeSTFT, kernelSz / 2 + 1, 0, kernelSz / 2 + 1);
      ...
      for (int i = 0; i < rangeSTFT; i += stepSz) {
        ...
        auto minIt =
            std::min_element(traces.begin() + i, traces.begin() + i + kernelSz);
        double minVal = *minIt;
        for (int j = 0; j < kernelSz; j++) {
          tracesShort[j] =
              gauss[j] * (traces[i + j] - minVal) * 100 /
              sumKernel; // the traces-minval makes the pulse to be only >0 and
                         // shifts the baseline accordingly
      ...
      projectionX = hSTFT->ProjectionX(
          "projX", 0, 1); // here the projection is taken to get peak positions
      // std::cout << "PROJECTION DONE" << std::endl;
      Int_t nfound = s->Search(projectionX, 2, "",
                               0.05); // sigma=2 bins, threshold=5% of max
      ...
      for (int numpeak = 0; numpeak < nfound; numpeak++) {
        // std::cout << xpeaks[numpeak] << " : " << ypeaks[numpeak] <<
        // std::endl;
        ...
        if (ypeaks[numpeak] > peakThreshSTFT) {
          ...
          std::cout << xpeaks[numpeak] << " : " << ypeaks[numpeak] << " : "
                    << peakBin << std::endl;
          int tracesLoc = 0;
          for (int timeval = std::max(peakBin - (kernelSz + 10), 0);
               timeval < std::min(peakBin + (kernelSz + 10), rangeSTFT);
               timeval++) {
      ...
      // // IFFT gives bipolar pulses if points set as c=(re,im) instead of
      // c=TComplex(re,im) (differential of the original pulse) hence
      // integrating it to get unipolar pulse double CDFtracesPeaks = 0; for
      // (int iter = 0; iter < N; iter++)
      // {
      ...
      //     {
      //         std::vector<double> tracesShort(kernelSz);
      //         auto minIt = std::min_element(tracesPeaksUni.begin() + iter,
      //         tracesPeaksUni.begin() + iter + kernelSz); double minVal =
      //         *minIt; for (int j = 0; j < kernelSz; j++)
      //         {
      //             tracesShort[j] = gauss[j] * (tracesPeaksUni[iter + j] -
      //             minVal) / sumKernel;
      //             // tracesShort[j] = (tracesPeaksUni[iter + j] - minVal) /
      //             100; tracesPeaksUni1[iter + j] += tracesShort[j];
      //         }
      //     }
      //     else
      ...
      for (int i = 0; i < N; ++i) {
        ...
        if (i < N - kernelSz)
          // graphTracesTH2->SetPoint(i, i, (tracesMod[i + kernelSz / 2] / 100 +
          // kernelSz / 2));
        ...
        // graphTracesPeaksTH2Fin->SetPoint(i, i, tracesPeaksUni[i] / 100 +
        // kernelSz / 4);
      }
