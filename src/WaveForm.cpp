...
namespace digiAnalysis
{
  ...
  UShort_t WaveForm::nSampleBL = static_cast<UShort_t>(std::stoul(env_var_baselinesamples));
  ...
  WaveForm::WaveForm(const std::vector<UShort_t> tr) // copy from other vector
  {
    ...
    {
      SetBaseLine(tr);
      if (std::fabs(baseline - tr[0]) > 100) // check if the copied traces have baseline subtracted
      {
        SetWaveForm(tr);
      }
      else
  ...
  WaveForm::WaveForm(UShort_t numWaveForm, UShort_t sizeOfWaveForms, const std::vector<WaveForm> vecOfWaveForm)
  {
  ...
  void WaveForm::SetSmooth(UShort_t smoothBoxSz)
  {
    ...
    {
      std::cout << "Fill traces first" << std::endl;
    }
  }
  ...
  void WaveForm::SetMeanTime()
  {
    ...
    {
      std::cout << "traces is empty" << std::endl;
    }
  }
  ...
  void WaveForm::SetMeanTime(const std::vector<UShort_t> tr, UShort_t start, UShort_t stop)
  {
    meantime = 0;
    float sampleSum = 0;
    if (!tr.empty())
    {
      if (start < stop)
      {
        if (tr.size() < stop)
        {
          std::cout << "Warning: stop is less than input vector size. Evaluating till end" << std::endl;
          stop = tr.size();
        }
      ...
      {
        std::cout << "error: start > stop, exiting witout meantime setting" << std::endl;
      }
    }
    else
    {
  ...
  void WaveForm::SetBaseLine(TArrayS *arr)
  {
    ...
    {
      std::cout << "input array is empty or smaller than NUM_SAMPLE_BASELINE" << std::endl;
    }
  }

  // std::vector<float> WaveForm::EvalDecayTime(UShort_t FitStart, UShort_t FitEnd, UShort_t numDecayConst) {}
  ...
  Long64_t WaveForm::IntegrateWaveForm(int startTime, int stopTime)
  {
    ...
    if (!traces.empty() && (traces.size() > stopTime))
    {
      ...
      {
        std::cout << "order the times properly " << std::endl;
      }
    }
    else
    {
  ...
  void WaveForm::ConcatenateWaveForms(const WaveForm &wf1, const WaveForm &wf2)
  {
    ...
    {
      tracesSmooth.insert(tracesSmooth.end(), wf2.tracesSmooth.begin(), wf2.tracesSmooth.end());
    }
    if (!wf2.CFDtraces.empty()) // concatenate traces CFD if not empty
    {
      CFDtraces.insert(CFDtraces.end(), wf2.CFDtraces.begin(), wf2.CFDtraces.end());
    }

    meantime = traces.size() + 1000; // can use this to check if the waveform is single or concatenated
    baseline = wf1.baseline;
  }

  void WaveForm::AverageWaveForms(UShort_t numWaveForm, UShort_t sizeOfWaveForms, const std::vector<WaveForm> vecOfWaveForm)
  {
