...
namespace digiAnalysis
{
  ...
  */
  ...
  void WaveForm::AverageWaveForms(ULong_t start, UShort_t numWaveForm, UShort_t sizeOfWaveForms,
                                  const std::vector<WaveForm> vecOfWaveForm)
  {
    // currently only traces is averaged
    float sum = 0;
  ...
  void WaveForm::ScaleWaveForm(double Scale)
  {
    ...
    if (!tracesSmooth.empty())
    {
      std::transform(tracesSmooth.begin(), tracesSmooth.end(), tracesSmooth.begin(),
                     [Scale](float val)
                     { return val * Scale; });
    }
  }
  ...
  void WaveForm::AddWaveForm(const WaveForm &wf1)
  {
    if (traces.size() == wf1.traces.size())
    {
      ...
      std::transform(traces.begin(), traces.end(), wf1.traces.begin(), traces.begin(),
                     std::plus<float>());
    }
    else
    {
      std::cout << "traces must be the same size for adding! Nothing done!!!!!!!!" << std::endl;
    }
    if (tracesSmooth.size() == wf1.tracesSmooth.size())
    {
      std::transform(tracesSmooth.begin(), tracesSmooth.end(), wf1.tracesSmooth.begin(), tracesSmooth.begin(),
                     std::plus<float>());
    }
    else
    {
